...
这个模块把原本只存在于脚本里的同步逻辑抽出来，供两种入口复用：
1. 命令行脚本手动执行。
2. FastAPI 接口在运行时触发一次同步。
"""
...
CONFIG_FILE = settings.base_dir / "scripts" / "sync_config.json"
_sync_lock = Lock()
...
def sync_tools_data() -> dict[str, Any]:
    ...
    write_json_atomic(payload, settings.data_file)
    logging.info("同步完成，已写入 %s 条记录到 %s", payload["total"], settings.data_file)
...
def trigger_sync() -> dict[str, Any]:
    ...
    if not acquired:
        raise SyncInProgressError("已有同步任务正在执行，请稍后再试。")

    try:
        return sync_tools_data()
    finally:
        _sync_lock.release()